 ...
 *     - Alphabet used in input strings consists of all visible characters only.
 ...
 * _Algorithms in C++_.
 *
 * slightly modified by Dominik Szopa
 */
...
/*	if(c == ' ')
    ...
    _offGraphics.setFont(new Font("Helvetica", Font.PLAIN, 12));
	...
	for(i = 0; i < ALPH_SIZE; i++) {
	    char_width = _offGraphics.getFontMetrics().charWidth(ALPH_STRING.charAt(i));
	...
	for(j = 0; j < ALPH_SIZE; j++) {
		...
		_offGraphics.drawString(charCode(j), dispRect.x + (_height*2), i);
		i += _height;
	    }
	}
    ...
    static final int ALPH_SIZE = 93;       // # chars in alphabet
